## Hi there 👋 

<!--
**cuteblacky/cuteblacky** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I'm Cornelius, a frontend developer based in Brazil. 
- 🌱 I design and build whatever i can imagine. 
 If an idea pops in my head, chances are i'll build it and ship it. 
- 👯 I’m looking to collaborate with other developers ...
- 😄 My main tech stack is react, typescript, tailwind CSS, Flutter, Framer motion. My favorite tools are Asana, Canva, figma, adobe creative collection.
- 💬 I've got a bunch of pinned projects.
- 📫 How to reach me: iamcuteblacky@gmail.com
- ⚡ Fun fact: ...
-->
 
